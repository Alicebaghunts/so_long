...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025-05-02 12:46:17 by alisharu          #+#    #+#             */
/*   Updated: 2025-05-02 12:46:17 by alisharu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/so_long.h"

static void	tank_move_right_animation(t_game *data)
{
...
static int	move_right(t_game *data)
{
	float	copy_x;
	float	copy_y;
	...
	if ((data->map->map[(int)floor(copy_y)][(int)floor(copy_x) + 1] == '1')
		|| (data->map->map[(int)floor(copy_y)][(int)floor(copy_x) + 1] == 'M')
		|| ((data->map->map[(int)floor(copy_y)][(int)floor(copy_x) + 1] == 'E')
		&& data->map->coin != 0))
		return (0);
	check_coin_and_exit(data, floor(copy_x) + 1, floor(copy_y));
	if (++data->player->frame_rate >= TANK_MOVE_ANIM_LIMIT)
	{
		...
		data->player->x += 8;
		copy_x = (float)data->player->x / (float)TILE_SIZE;
		tank_move_right_animation(data);
